...
version_pattern="^${numeric}\.${numeric}\.${numeric}(-${identifier}(\.${identifier})*)?(\\+${identifier}(\.${identifier})*)?$"
t_assert_eq 'package.json states a version' \
    "$(printf '%s' "$version" | grep -Ec "$version_pattern")" '1'

for valid in 2.0.0-alpha 2.0.0-alpha.1 2.0.0+build.7; do
    t_assert_eq "valid SemVer is accepted: $valid" \
        "$(printf '%s' "$valid" | grep -Ec "$version_pattern")" '1'
done
for invalid in 2.0 x.y.z '' 02.0.0 2.0.0- 2.0.0-alpha..1; do
