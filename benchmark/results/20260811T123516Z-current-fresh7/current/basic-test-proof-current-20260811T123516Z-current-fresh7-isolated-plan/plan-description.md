...
§ 3.1
A future executor can create button-chain.html with one initial button, append exactly one button below the current last button when that last button is pressed, and clear the document when the fourth generated button is pressed.

§ 3.2
The completion state must render the exact lowercase text finished with a visible white border. The plan is complete when all durable planning artifacts, UI story contracts, testing companions, review evidence, validation output, and benchmark analysis are present.
