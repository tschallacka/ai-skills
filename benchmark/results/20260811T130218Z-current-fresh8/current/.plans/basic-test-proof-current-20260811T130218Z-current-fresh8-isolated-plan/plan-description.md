...
§ 3.1
A future executor can create button-chain.html with one initial button. Each click on the current last button appends exactly one new button below it, and clicking the fourth generated button clears the document and shows the exact lowercase text finished with a visible white border.
