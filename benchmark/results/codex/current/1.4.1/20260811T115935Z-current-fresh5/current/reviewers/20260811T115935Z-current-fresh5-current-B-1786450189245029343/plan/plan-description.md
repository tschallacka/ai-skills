...
§ 3.1
A future executor can create button-chain.html with one initial visible button. Pressing the current last button appends exactly one new button below it, and pressing the third generated button clears the document and displays the exact lowercase text finished with a visible black border.
...
§ 5.1
In scope: one future file named button-chain.html, two initial buttons, vertical appended generated buttons, exact one-button append behavior, terminal clearing on the fourth generated button, exact lowercase completion text, visible white border, and browser verification instructions.
