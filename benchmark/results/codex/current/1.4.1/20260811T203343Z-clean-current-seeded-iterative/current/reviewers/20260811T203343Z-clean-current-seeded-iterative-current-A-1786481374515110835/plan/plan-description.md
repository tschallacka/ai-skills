...
§ 3.1
A future executor can create button-chain.html from this plan without reconstructing scope. The completed implementation must show two initial buttons; pressing the current last button must append exactly one button below it; pressing the third generated button must clear the document and show exactly finished in lowercase with a visible black border.
