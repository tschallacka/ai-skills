...
§ 3.1
Produce a durable implementation plan for a future button-chain.html task. The future deliverable loads with exactly one initial button; clicking only the current last button appends exactly one new button below it; after the fourth generated button exists, pressing that fourth generated button clears the document and renders exact lowercase text finished with a visible white border.
