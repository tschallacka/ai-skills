...
§ 3.1
A future executor can create button-chain.html with two initial buttons, append exactly one button below the current last button when that last button is pressed, clear the document when the third generated button is pressed, and display the exact lowercase text finished with a visible black border.
