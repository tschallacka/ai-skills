...
for defect in defects:
    required = ("id", "path", "location", "expected_signal", "required_correction", "severity", "defective_sha256")
...
def correction_matches(finding, defect):
    ...
    if expected in correction:
        return True
    expected_words = tokens(expected)
    return bool(expected_words) and len(expected_words & tokens(correction)) * 2 >= len(expected_words)
...
for defect in defects:
    ...
    incomplete = []
    for index, finding in enumerate(findings):
        if not isinstance(finding, dict) or candidate_path(finding) != norm(defect["path"]).lstrip("./") or not location_matches(finding, defect):
            continue
    ...
    if len(targeted) > 1:
        ...
        classification, selected = ("ambiguous" if any(findings[i].get("ambiguous") is True for i in incomplete if isinstance(findings[i], dict)) else "unresolved"), incomplete
    else:
        classification, selected = "false_positive", []
    used_findings.update(selected)
    if classification == "true_positive" and any(findings[i].get("independent") is True for i in selected):
        independent_catches += 1
    rows.append({"defect_id": defect["id"], "finding_ids": [findings[i].get("finding_id") for i in selected if isinstance(findings[i], dict) and required_string(findings[i].get("finding_id"))], "classification": classification, "confidence": "high" if classification == "true_positive" else "low", "rationale": "independent semantic adjudication"})

private_rows.write_text(json.dumps(rows, indent=2, sort_keys=True) + "\n", encoding="utf-8")
counts = {"true_positives": sum(row["classification"] == "true_positive" for row in rows), "false_positives": sum(row["classification"] == "false_positive" for row in rows) + max(0, len(findings) - len(used_findings)), "duplicates": sum(row["classification"] == "duplicate" for row in rows), "unresolved": sum(row["classification"] == "unresolved" for row in rows), "ambiguous": sum(row["classification"] == "ambiguous" for row in rows), "false_negatives": sum(row["classification"] != "true_positive" for row in rows), "independent_catches": independent_catches}
