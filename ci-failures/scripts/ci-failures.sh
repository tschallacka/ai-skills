...
run_gh() {
    ...
    #
    # The order matters. A bare number is ambiguous -- run ids and PR numbers
    # are both integers -- and the disambiguation is by MAGNITUDE, which is a
    # heuristic and therefore stated out loud rather than hidden: a GitHub
    # Actions run id is a 10+ digit snowflake, a PR number in this repository
    # is two or three digits. `pr/47` says which is meant when that guess is
    # not good enough.
    gh_resolve_run() {
        ...
        case "$want" in
            pr/*)
                ...
                gh_latest_run_for "$head_branch"
                return
                ;;
            '')
                branch="$(git symbolic-ref --short -q HEAD || true)"
                [ -n "$branch" ] || { printf 'ci-failures: detached HEAD; name a run, PR or branch\n' >&2; exit 64; }
                gh_latest_run_for "$branch"
                return
                ;;
        esac
        case "$want" in
            *[!0-9]*)
                gh_latest_run_for "$want"
                return
                ;;
        esac
        if [ "${#want}" -ge 9 ]; then
    ...
    # The newest run for a branch, ignoring the artifact-render workflow: it
    # is almost always green and never the reason a check suite is red.
    gh_latest_run_for() {
        local branch="$1" id
    ...
    printf '%s\n' "$jobs" | while IFS="$(printf '\t')" read -r job_id job_conclusion job_name; do
        ...
        # terminal colour codes, and every CI log carries them. Without it
        # this prints nothing at all and the run looks empty.
...
run_glab() {
    ...
    # "namespace/project" path there; the path is what a remote URL gives us
    # for free, so that is what gets computed rather than a second lookup).
    #
    ...
    # stable, which is exactly why the gh path above prefers `gh api` over
    # `gh run view`'s own formatting too. This path has never been run against
    # a live GitLab project (see VERIFIED DIFFERENTLY above) -- it is written
    # to the documented contract and exercised in tests against a stub.
    glab_project_path() {
        ...
        # Two shapes, told apart by whether a scheme is present, not stripped
        # the same way: a URL form (https://host/group/sub/project.git) has
        # the project path after the FIRST slash past the host, while an
        # scp-like form (git@host:group/sub/project.git) has no host slash to
        # strip at all -- applying both stripping steps to the scp-like form
        # ate its first path segment as if it were a host.
        case "$url" in
            *://*)
                path="${url#*://}"
                path="${path#*/}"
    ...
    # Resolution mirrors gh_resolve_run: pr/N and a bare small number both name
    # a merge request (GitLab's MR !iid), a bare large number names a pipeline
    # id directly, empty names the current branch. The magnitude threshold is
    # the same heuristic as the gh path, for the same reason: stated out loud,
    # not hidden, and `pr/N` (kept as the spelling on both forges, rather than
    # asking for `mr/N` here) says which is meant when the guess is not good
    # enough.
    glab_resolve_pipeline() {
