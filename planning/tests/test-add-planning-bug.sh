...
# The bash fallback that used to shell out to rjq for the append is gone (T145),
# and with it the "rjq answered the append with nothing" failure mode this file
# once forced: the compiled add-planning-bug builds and serializes the register
# in-process, so a write that succeeds and leaves the entry out has no seam left
# to reproduce it through.
