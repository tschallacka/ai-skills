...
plan_validate_propagation_symbols_token() { # <id> <token> <edit_lines>
    ...
    local klass klass_short prefix_match prefix owned candidate file_cell scope_cell scope_class scope_class_short
    ...
    # Condition 1: the symbol's namespace root must be a project prefix the
    # plan edits (vendor seams drop out here by construction).
    klass="${token%%::*}"
    klass_short="${klass##*\\}"
    prefix_match=false
...
plan_validate_propagation_roster_goal() { # <goal_name>
    local goal_name="$1" goal_file assigned roster_ids blurb_ids id rid aid
    ...
    # goal. Ids not in the plan at all are skipped; cross-plan references sit
    # after the em-dash and are already out of the leading run.
