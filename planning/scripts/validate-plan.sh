...
# --- "the application still serves" check (report 17 §1, P0): a goal that
#     changes module state, schema, or configuration must verify the running
#     application, not just the changed artifact. Registry-driven like the
#     placeholder check: planning/state-change-registry.json holds the exact
#     indicator and serve phrases. A state-changing goal whose verification
#     units carry no serve phrase gets a WARN (the report asks for a
#     convention with a warn, not a hard gate).
state_change_registry="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/state-change-registry.json"
if [ -f "$state_change_registry" ]; then
    mapfile -t state_indicators < <(jq -r '.indicators[]?' "$state_change_registry" 2>/dev/null || true)
    mapfile -t serve_phrases < <(jq -r '.serve_phrases[]?' "$state_change_registry" 2>/dev/null || true)
    for goal_name in "${!goal_units[@]}"; do
        touched=false
        for id in ${goal_units[$goal_name]:-}; do
            step_file="$plan_dir/$goal_name/steps/${unit_step[$id]}.md"
            for ind in "${state_indicators[@]}"; do
                [ -n "$ind" ] || continue
...
# --- command registry (report 17 §2, P1; report 20 rules): every command
#     literal in step files and testing companions must be registered in the
#     plan's commands.json (seeded empty by create-plan.sh, maintained with
#     register-command.sh), so the "when" context travels with the command
#     instead of being lost when steps are copied. Detection is
#     language-agnostic (reports 20 §4 + 21 §3, ordered): a span is a
#     candidate when (1) its first token is a registered command's first token
#     or a universal core word, or (2) it resolves on disk to a non-directory
#     with the executable bit — the same question a shell would ask — or (3)
#     its last segment sits directly under a bin-like directory (bin/, sbin/,
#     .bin/, Scripts/, covering vendor/bin/, node_modules/.bin/, .venv/bin/).
#     Rules 1-3 are the only entry points: arguments strengthen a qualifying
#     span but never qualify one on their own (report 21 §3). It is
#     disqualified regardless when (5) its last segment carries a
#     never-executable data/markup extension (the jq-matched list in
#     never-executable-extensions.json), or (6) it ends in a :line/#Lnn
#     (7) it is route- or prose-shaped (a leading / without a bin-like segment
#     whose first token is not command-shaped, or a leading-/ argument after a
#     non-command-shaped token, e.g. `GET /health`), or (8) its first token
#     resolves to a directory. The vocabulary is the registry itself: each
#     registered command's first token teaches the detector that tool word
#     (report 20 §3), so a Python plan registering `pytest -q` makes pytest a
#     word with no language table to maintain. WARN mid-draft; FAIL under
#     --complete, like authored placeholders.
...
if [ -f "$commands_file" ]; then
    ...
    registered_words="$(jq -r '. | to_entries[] | .value.cmd' "$commands_file" 2>/dev/null \
        | awk '{ print $1 }' | sort -u)"
    extensions_file="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/never-executable-extensions.json"
    ...
    for id in "${unit_ids[@]}"; do
        for candidate_file in \
            "$plan_dir/${unit_goal[$id]}/steps/${unit_step[$id]}.md" \
            "$plan_dir/${unit_goal[$id]}/steps/${unit_step[$id]}-testing.md"; do
