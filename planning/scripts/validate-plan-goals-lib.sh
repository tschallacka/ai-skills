...
# plan_require_ticked_atomicity STEP_FILE PLAN_DIR GOAL STEP — relaxed
# atomicity gate (W07): the three boxes must be present; ticked boxes may
# carry an annotation suffix (auto-tick evidence); unticked boxes fail only
# when the step is already completed in its goal progress tracker.
plan_require_ticked_atomicity() {
    local step_file="$1" plan_dir="$2" u_goal="$3" u_step="$4"
    local sent tick line completed=no
...
# validate_goal_testing_band PLAN_DIR GOAL UNIT_IDS — the test-unit tally and
# companion-file requirement for one goal: testing required needs at least one
# test/verification unit, a test unit needs the requirement, and non-docs
# units in a testing goal need their -testing.md companion.
validate_goal_testing_band() {
    local plan_dir="$1" goal_name="$2" goal_unit_ids="$3"
    local id test_units=0 goal_required companion
...
# require_goal_headings GOAL_FILE — every mandated narrative heading present.
require_goal_headings() {
    local goal_file="$1" heading
    for heading in \
...
# validate_goal_size_band GOAL_FILE GOAL_NAME COUNT UNIT_IDS — the one-unit
# exception shape and the ten-unit split ceiling.
validate_goal_size_band() {
    local goal_file="$1" goal_name="$2" count="$3" goal_unit_ids="$4"
    local only_id only_type
    if [ "$count" -eq 1 ]; then
...
# require_step_headings STEP_FILE — the seven mandated step sections present.
require_step_headings() {
    local step_file="$1" heading
    for heading in '## Ownership' '## Change target' '## Objective' \
        '## Instructions' '## Acceptance criteria' '## Handoff' \
        '## Atomicity check'; do
        require_heading "$step_file" "$heading"
    done
}
...
plan_validate_goals() {
    ...
    for goal_dir in "$plan_dir"/[0-9][0-9]-*/; do
        ...
        require_goal_headings "$goal_file"
        ...
        validate_goal_size_band "$goal_file" "$goal_name" "$count" "$goal_unit_ids"
        validate_goal_testing_band "$plan_dir" "$goal_name" "$goal_unit_ids"
    done
}
...
plan_validate_step_files() {
    for id in ${unit_ids[@]+"${unit_ids[@]}"}; do
        ...
        require_step_headings "$step_file"
        ...
        plan_require_ticked_atomicity "$step_file" "$plan_dir" "$u_goal" "$u_step"
    done
}
