...
# Sets touched (true/false) and goal_unit_ids for this goal. The indicator
# scan's `break 2` leaves the unit loop from inside the indicator loop, so
# both nests must stay inside this one function.
plan_validate_still_serves_touched() { # <goal_name>
    local goal_name="$1" id step_file ind
    touched=false
...
# Sets served (true/false) for goal_name/goal_unit_ids (both left by
# plan_validate_still_serves_touched). Same break-2 constraint as above, this
# time over the phrase scan.
plan_validate_still_serves_served() {
    local id step_file acceptance acceptance_lc phr phr_lc
    served=false
    for id in $goal_unit_ids; do
