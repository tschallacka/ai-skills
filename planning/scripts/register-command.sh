...
# rjq is a declared runtime requirement, but a hand-copied skill directory can
# still miss it; say so with the "tool unavailable" code rather than letting the
# shell's own "command not found" surface.
command -v rjq >/dev/null 2>&1 || plan_die 'rjq is required by register-command.sh; install rjq (macOS: brew install rjq, Debian: apt-get install rjq)' 69
...
# The trap is installed before the first write so a rjq failure cannot leak the
# temp, and is never released with `trap - EXIT`: that would discard the
# library's cleanup handler too (CODE-STYLE §8).
case "$mode" in
    list)
        rjq -r 'to_entries[] | "\(.key)\t\(.value.cmd)\t\(.value.when)"' "$commands_file"
        exit 0
        ;;
    remove)
        tmp_file="$(mktemp "${TMPDIR:-/tmp}/plan-command-register.XXXXXX")"
        trap 'rm -f "$tmp_file"' EXIT
        rjq --arg k "$key" 'del(.[$k])' "$commands_file" > "$tmp_file"
...
tmp_file="$(mktemp "${TMPDIR:-/tmp}/plan-command-register.XXXXXX")"
trap 'rm -f "$tmp_file"' EXIT
rjq --arg k "$key" --arg c "$command_literal" --arg w "$when" \
