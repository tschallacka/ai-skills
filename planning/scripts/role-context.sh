...
tmp="$(mktemp "${TMPDIR:-/tmp}/role-context.XXXXXX")"; trap 'rm -f "$tmp"' EXIT
{
    ...
    for rel in $(role_docs "$id"); do
        file="$skill_dir/$rel"
        if [ -f "$file" ]; then
            if [ "$rel" = REVIEWER.md ]; then
                # REVIEWER.md pins the whole-file hash of skill-source.txt (T87:
                # moved off SKILL.md, which is now a generated index); the copy
                # on disk can go stale after a source edit because nothing
                # committed carries the fresh one. Verify the pin at read time
                # and name the generator when it lags. The hash probe mirrors
                # plan_sha256_hex's order (plan-crypt binary, GNU, BSD) without
                # sourcing the compiled library, which is itself generated and
                # may be absent on a tree that has not been bootstrapped.
                reviewer_pin="$(sed -n 's/^> Source SHA-256: `\([0-9a-f]*\)`.*/\1/p' "$file" | head -1)"
                skill_hash=''
                if command -v sha256sum >/dev/null 2>&1; then
                    skill_hash="$(sha256sum "$skill_dir/skill-source.txt" | awk '{print $1}')"
                elif command -v shasum >/dev/null 2>&1; then
                    skill_hash="$(shasum -a 256 "$skill_dir/skill-source.txt" | awk '{print $1}')"
                fi
                if [ -n "$skill_hash" ] && [ -n "$reviewer_pin" ] \
                    && [ "$reviewer_pin" != "$skill_hash" ]; then
                    printf '\n# (stale: %s pins an older skill-source.txt — regenerate with scripts/generate-reviewer.sh)\n' "$rel"
                    continue
                fi
            fi
            printf '\n===== %s =====\n' "$rel"
            cat "$file"
        elif [ "$rel" = REVIEWER.md ]; then
            printf '\n# (missing: %s — generated by scripts/generate-reviewer.sh from skill-source.txt; run it and re-run this command)\n' "$rel"
        else
            printf '\n# (missing: %s)\n' "$rel"
        fi
    done
