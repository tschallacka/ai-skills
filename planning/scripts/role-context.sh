    ...
    for rel in $(role_docs "$id"); do
        ...
        if [ -f "$file" ]; then
            if [ "$rel" = REVIEWER.md ]; then
                # REVIEWER.md pins the whole-file hash of SKILL.md; the copy on
                # disk can go stale after a SKILL.md edit because nothing
                ...
                if command -v sha256sum >/dev/null 2>&1; then
                    skill_hash="$(sha256sum "$skill_dir/SKILL.md" | awk '{print $1}')"
                elif command -v shasum >/dev/null 2>&1; then
                    skill_hash="$(shasum -a 256 "$skill_dir/SKILL.md" | awk '{print $1}')"
                fi
                if [ -n "$skill_hash" ] && [ -n "$reviewer_pin" ] \
                    && [ "$reviewer_pin" != "$skill_hash" ]; then
                    printf '\n# (stale: %s pins an older SKILL.md — regenerate with scripts/generate-reviewer.sh)\n' "$rel"
                    continue
                fi
            fi
            printf '\n===== %s =====\n' "$rel"
            ...
            printf '\n# (missing: %s — generated by scripts/generate-reviewer.sh from SKILL.md; run it and re-run this command)\n' "$rel"
        else
            printf '\n# (missing: %s)\n' "$rel"
        fi
    done
