...
# plan_content_diff_paragraph_labels <changed-document> — the §-labels a
# document's changed lines fall under. Uses the caller's $repo_root, $git_ref,
# $plan_rel, $plan_abs (bash's dynamic scoping makes the caller's locals
...
# gsub: POSIX leaves a backslash in a gsub replacement undefined unless it
# precedes "&" or "\", and busybox awk really does swallow the one in
# "\\u00a7".
plan_content_diff_json_escape() {
    printf '%s' "$1" | awk '
        {
            backslash = "\\"
            escaped = ""
...
# plan_content_diff_emit_json <changed> — $git_ref and the paragraph-label
# helper come from the caller's scope (dynamic scoping).
plan_content_diff_emit_json() {
    local changed="$1" changed_document document_first label_first paragraph_label
    printf '{"ref": "%s", "documents": [' "$(plan_content_diff_json_escape "$git_ref")"
    document_first=true
    while IFS= read -r changed_document; do
        [ -n "$changed_document" ] || continue
        [ "$document_first" = true ] || printf ', '
        document_first=false
        printf '{"document": "%s", "paragraphs": [' "$(plan_content_diff_json_escape "$changed_document")"
        label_first=true
        while IFS= read -r paragraph_label; do
            [ -n "$paragraph_label" ] || continue
            [ "$label_first" = true ] || printf ', '
            label_first=false
            printf '"%s"' "$(plan_content_diff_json_escape "$paragraph_label")"
        done <<< "$(plan_content_diff_paragraph_labels "$changed_document")"
        printf ']}'
    done <<< "$changed"
    printf ']}\n'
}

plan_content_diff_emit_text() {
    local changed="$1" changed_document
    while IFS= read -r changed_document; do
        [ -n "$changed_document" ] || continue
        printf '## %s\n' "$changed_document"
        plan_content_diff_paragraph_labels "$changed_document"
    done <<< "$changed"
}
...
plan_content_diff() {
    ...
    if [ "$format" = json ]; then
        plan_content_diff_emit_json "$changed"
    else
        plan_content_diff_emit_text "$changed"
    fi
}
