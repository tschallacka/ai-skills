...
# plan change, Status). Reads CSV from stdin by default (heredoc-friendly) or
# from --file PATH. It only rewrites Findings; the Verdict is authored by the
...
if [ -n "$csv_source" ]; then
    ...
    if [ -t 0 ]; then
        plan_err "no CSV provided. Pipe rows or a heredoc to stdin, or pass --file PATH (columns: ID, Missing or over-broad item, Required plan change, Status)"
    fi
    cat > "$csv_file"
fi
[ -s "$csv_file" ] || plan_err "CSV input is empty (columns: ID, Missing or over-broad item, Required plan change, Status)"

plan_render_csv_table 4 "$(awk '{ printf "%s\\n", $0 }' "$csv_file")" > "$rendered_file"
