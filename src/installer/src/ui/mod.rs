...
//! input and terminal together; see each submodule's own doc comment for
//! what it ports and what it deliberately leaves out (no mascot, no
//! dependency table, no mouse, no integration-mode cycling).
...
pub fn run_picker(skills: Vec<SkillEntry>) -> Option<Vec<String>> {
    ...
    loop {
        ...
        let layout = layout::compute(cols, rows, &names);
