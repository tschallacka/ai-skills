...
mod tests {
    ...
    /// actual `bash` subprocess ONLY here, as an oracle this test compares
    /// the production (no-bash) parser's answer against. `bash` missing on
    /// PATH skips this test rather than failing it: it is a stronger check
    /// on top of the unit tests above, not the only coverage this parser has.
    #[test]
    fn matches_install_shs_own_answer_for_every_real_skill_in_this_repo() {
        let repo_root = {
