...
// B343: update-plan-content.sh's -rv only ever writes one of these two exact
// literals ("✅ approved" / "💤 pending", per update_review_status) once
// extract.rs's own backtick-stripping is correct -- never the bare words
// these fixtures used before, which masked derive_mode comparing against the
// wrong literal too.
#[test]
