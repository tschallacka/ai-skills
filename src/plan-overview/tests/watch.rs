...
fn temp() -> PathBuf {
    let nonce = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("overview-watch-{}-{nonce}", std::process::id()));
...
fn coalescing_and_scan() {
    ...
    let outside = ChangeEvent {
        changed: vec![std::env::temp_dir().join("outside")],
    ...
    assert_eq!(coalesce_events(&[event, outside], DEBOUNCE_WINDOW).len(), 2);
    fs::write(root.join("plan.md"), "two").unwrap();
    assert!(scan_plan_dir(&root, &second).unwrap().1.is_none());
    fs::write(std::env::temp_dir().join("outside"), "x").unwrap();
    assert!(snapshot(&root).unwrap().file_count() > 0);
    let _ = fs::remove_file(std::env::temp_dir().join("outside"));
