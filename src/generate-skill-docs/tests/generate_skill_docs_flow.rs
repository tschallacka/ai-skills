...
// end to end (exact exit code, exact stderr message) -- the same pattern
// src/build-plan-libs/tests/build_plan_libs_flow.rs established for W58.
use std::env;
use std::fs;
...
fn relocated_binary(tag: &str) -> PathBuf {
    let source = PathBuf::from(env!("CARGO_BIN_EXE_generate-skill-docs"));
    let mut outside = env::temp_dir();
    outside.push(format!(
        "generate-skill-docs-flow-{tag}-{}-{:?}",
        std::process::id(),
...
fn skill_root_failure_path_exits_69_with_the_exact_message() {
    ...
    assert_eq!(
        stderr.trim_end(),
        "generate-skill-docs.sh: could not locate the planning skill root"
    );
