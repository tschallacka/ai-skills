...
# The two tables below were generated by installer/build.sh, which went with the
# shell installer; nothing generates them now and the block is empty. The
# compiled installer reads each skill's integration.tsv itself (see
# src/installer/src/integration.rs), so edit those files.
#
...
#
# Kept here, beside the INTEGRATION_* tables it reads; 50-manifest.sh calls it.
# The compiled installer has its own port in src/installer/src/integration.rs.
